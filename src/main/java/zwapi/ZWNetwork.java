...
public class ZWNetwork implements SerialPortEventListener {
     ...
     *            The static controller for the z-wave network. aka the usbstick.
     */
    ...
    public ZWNode removeNode(ZWNode node) {
        ...
        return this.nodes.remove(node.getID());
    }
    ...
    public ZWFrame sendData(byte dst, byte[] data) {
        ...
        this.networkController.send(zm.getBytes());
        return zm;
    }
    ...
    public void serialEvent(SerialPortEvent arg0) {
        ...
        try {
            ...
            while ((data = this.networkController.getInputStream().read()) > -1) {
                ...
                if (message == null) {
                    ...
                    if (done) {
                        ...
                        this.networkController.sendAck();
                        ZWCmdClass cmd;
                        switch (message.command) {
                            case 0x15: //getVersion response
                                ZWEvent event = new ZWEvent(this.networkController, (byte) 0x86, (byte) 0x12, message.payload);
                                this.networkController.fireZWVersionEvent(event);
                                break;
    ...
    public void fireEvent2(byte command, byte[] payload) {
        if (payload[0] == -124) {
            ...
            if (this.nodes.containsKey((int) payload[1])) {
                ZWNode node = this.nodes.get((int) payload[1]);
    ...
    public void fireEvent(ZWCmdClass cmd) {
        ZWEvent event = new ZWEvent(this.nodes.get((int) cmd.node), cmd.cmdClass, cmd.cmd, cmd.data);
        switch (cmd.cmdClass) {
            case (byte) 0x20: //basic
                this.nodes.get((int) cmd.node).fireZWBasicEvent(event);
                break;
            case (byte) 0x70: // configuration
                ((ZWSensor) this.nodes.get((int) cmd.node)).fireZWConfigurationEvent(event);
                break;
            case (byte) 0x85: // association
                ((ZWSensor) this.nodes.get((int) cmd.node)).fireZWAssociationEvent(event);
                break;
            case (byte) 0x31: //sensor multilevel
                ((ZWEZMotionSensor) this.nodes.get((int) cmd.node)).fireZWSensorMultilevelEvent(event);
                break;
            case (byte) 0x26: // switch multilevel
                ((ZWEZMotionSensor) this.nodes.get((int) cmd.node)).fireZWSwitchMultilevelEvent(event);
                break;
            case (byte) 0x84: // wake up
                ((ZWEZMotionSensor) this.nodes.get((int) cmd.node)).fireZWWakeUpEvent(event);
                break;
            case (byte) 0x86: // version
                this.nodes.get((int) cmd.node).fireZWVersionEvent(event);
                break;
        }
    }
}
