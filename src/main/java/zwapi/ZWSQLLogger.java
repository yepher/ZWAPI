...
public class ZWSQLLogger implements ZWBasicListener, ZWNodeInfoListener {
    ...
    public ZWSQLLogger() {
        try {
            ...
            this.conn = DriverManager.getConnection("jdbc:mysql://localhost/kiiib?user=KIIIB&password=42");//connect to the database
    ...
    public void ZWBasicEventOccurred(ZWEvent event) {
        ...
        int id = sensor.getID();
        System.out.printf("[%s] Sensor%d fired!\n", this.df.format(new Date()), id);
        sensorEvent(id);
    }

    private void sensorEvent(int sensorId) {
        try {
            this.stmt.executeUpdate("INSERT INTO sensor_events VALUES(" + sensorId + ",NOW())");
        } catch (SQLException se) {
            System.out.println("SQLException: " + se.getMessage());
            System.out.println("SQLState: " + se.getSQLState());
            System.out.println("VendorError: " + se.getErrorCode());
    ...
    public void ZWNodeInfoEventOccurred(ZWEvent event) {
        ...
        int id = source.getID();
        System.out.printf("[%s] Switch%d ", this.df.format(new Date()), id);
        switchEvent(id);
    }

    public void switchEvent(int switchId) {
        try {
            ResultSet result = this.stmt.executeQuery("SELECT id, status, timestamp, "
            ...
            if (result.next() && result.getLong("diff") < 3) {
                String query = String.format(
                        "UPDATE switch_events SET status = 0 WHERE id = %d AND timestamp = '%s'",
                        switchId,
