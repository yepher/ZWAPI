...
public class ZWFrame {
    ...
    public boolean digest(byte data) {
        if (this.count < 0) {
            ...
            this.payload[this.count - 3] = data;
        }
        this.count++;
        return false;
    }
    ...
    public byte[] getBytes() {
        ...
        int index = 0;
        bytes[index++] = SOF;
        bytes[index++] = this.length;
        bytes[index++] = this.type;
        bytes[index++] = this.command;
        if (this.payload != null) {
            System.arraycopy(this.payload, 0, bytes, index, this.payload.length);
        ...
        if (this.hasFuncID) {
            bytes[index++] = this.funcID;
        }
        bytes[index++] = this.checksum;
        return bytes;
    }
    ...
    public String getPayloadBytesString() {
        ...
        StringBuilder sb = new StringBuilder(5 * this.payload.length);
        sb.append(String.format("0x%02X", this.payload[0]));
        for (int i = 1; i < this.payload.length; i++) {
            sb.append(String.format(",0x%02X", this.payload[i]));
        }
